...
class EnviromentSpec extends ObjectBehavior
{
    ...
    public function it_should_set_WP_CLI_CONFIG_PATH_to_wp_cli_local_yml_if_there_is_no_argument_passed()
    {

        $cwd = getcwd();

        $config = $cwd . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'wp-cli.local.yml';
        ...
        $this->run(null);
        $this->shouldHaveEnv($cwd . DIRECTORY_SEPARATOR . "config" . DIRECTORY_SEPARATOR . "wp-cli.local.yml");
